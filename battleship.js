...
function Battleship(){
    ...
    for (var i in Object.keys(this.ships))
    {
        this.health+=this.ships[i].size;
    }
}
...
Battleship.prototype.createboard=function(){
    ...
    {
        var tmp=new Array();
        for (var j=0; j<Battleship.size;i++)
        {
            tmp[j]=0;
        }
        ret[i]=tmp;
...
Battleship.prototype.getHitConfirm(hit)
{
    if(hit.hit)
    {
...
Battleship.prototype.placeShipt(shipname,x,y,dir)
{
    switch (dir)
    {
        case 0:
